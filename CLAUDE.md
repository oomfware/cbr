@oomfware/cbr is a CLI tool to ask questions by browsing the web using Claude Code in read-only mode
with access to a Playwright instance.

the source code is organized under `src/`:

- `index.ts`: CLI entry point, composes commands
- `client.ts`: standalone Unix socket client, bundled to `dist/browser.mjs` (invoked by the
  `bin/browser` shim in each session)
- `commands/`: command definitions (ask, clean)
- `lib/`: shared utilities (IPC server, browser commands, snapshot system, session setup, paths)
- `assets/`: system prompt for the Claude Code agent
...
use the local build to test browser automation:

    mise exec -- node ./dist/index.mjs ask [options] <task>
...
useful repositories for cbr development:
...
- `github.com/microsoft/playwright` for Playwright API, ariaSnapshot, locator methods
...
before reaching for cgr, check if there are alternative means of sourcing the answers. see if a
package ships with raw sources or TypeScript declaration files, or website/documentation sites
listed in the manifest that may provide a more helpful source.
